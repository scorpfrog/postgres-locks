...
@Testcontainers
@SpringBootTest
@ContextConfiguration(initializers = {AdvisoryLockTest.Initializer.class})
public class AdvisoryLockTest {
    ...
    @Test
    void advisoryTryLock() throws InterruptedException {
    ...
    @Test
    void advisoryLockAllowsQuerying() throws InterruptedException {
    ...
    @Test
    void advisoryLockAllowsUpdate() throws InterruptedException {
        ...
        // Now try to update user 1 transactions and it should be OK
        List<AccountTransactionEntity> transactions = transactionRepository.findByUserId(1L);
        transactions.forEach(transactionEntity -> transactionEntity.setAmount(500D));
        transactionRepository.saveAll(transactions);
        System.out.println(transactionRepository.findByUserId(1L));
    }

    @Test
    void advisoryLockUpdateThroughLockNotAllowed() throws InterruptedException {

        Runnable runnable = new Runnable() {
            @Override
            public void run() {
                try {
                    accountTransactionService.lockUserTransactions(1L);
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
            }
        ...
        accountTransactionService.lockAndUpdate(1L);
        System.out.println(transactionRepository.findByUserId(1L));
    }
}
